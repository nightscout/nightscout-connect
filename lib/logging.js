...
const requestError = require('./machines/request-error');
...
function createLogger (debug) {
  ...
  function diagnostic (level, message, error) {
    // One definition of an error's HTTP status, shared with the actors.
    const status = requestError.status(error);
