...
function nightscoutSource (opts, axios) {
  ...
  console.log("NIGHTSCOUT SOURCE CONFIGURED");
  var default_headers = {
  ...
  function fetchCollection (session, last_known, collection, path, field) {
    ...
    console.log("FETCHING NIGHTSCOUT COLLECTION", collection);
  ...
  var impl = {
    authFromCredentials(creds, settings) {
      ...
      console.log("CHECKING NIGHTSCOUT SOURCE AUTH");
      return http.get(checkURL).then((resp) => {
        console.log("CHECKED NIGHTSCOUT SOURCE AUTH", { status: Number.isInteger(resp.status) ? resp.status : 'unavailable' });
        ...
        console.log("NIGHTSCOUT SOURCE AUTH FAILED", { status: Number.isInteger(status) ? status : 'unavailable' });
        if (apiSecret) {
          return getOrCreateReaderToken( );
        }
