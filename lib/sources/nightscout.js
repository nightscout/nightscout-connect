...
function nightscoutSource (opts, axios) {
  ...
  var impl = {
    ...
    align_to_glucose (last_known) {
      ...
      var buffer_lag = 25000; // 25 second buffer
      var jitter = Math.floor(Math.random( ) * 6); // 5 second random
    ...
    dataFromSesssion(session, last_known) {
      ...
      var query = { find: { dateString: { $gte: last_glucose_at.toISOString( ) } }, count };
