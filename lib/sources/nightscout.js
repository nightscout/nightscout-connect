...
function nightscoutSource (opts, axios, log) {
  ...
  // Profiles are read in bounded pieces, not all of them on every poll.
  //
  // /api/v1/profile.json ignores `find` and answers the newest `count`
  // profiles by startDate. /api/v1/profiles.json applies `find`, but when the
  // query does not constrain startDate Nightscout adds startDate >= now - 4
  // days, so every query below names startDate explicitly.
  //
  // With no profile bookmark (a sink that stores no profiles yet) the source
  // reads the profiles that apply to the window the other collections copy
  // (the same 2 days): every profile starting in it, and the one in effect when
  // it starts. After that each poll reads the profiles created or saved since
  // the bookmark, whatever their startDate (the profile editor sets created_at
  // when it saves), and the newest profile, so a change to the profile in use
  // that leaves created_at alone (an API PUT) is seen too.
  var EPOCH = new Date(0).toISOString( );
  var profilesQueryable = true;
  function getProfiles (session, path, params) {
    return http.get(path, { params, headers: authHeaders(session) }).then((resp) => Array.isArray(resp.data) ? resp.data : [ ]);
  }
  function fetchProfiles (session, last_known) {
    if (!collectionSet.has('profiles')) {
      return Promise.resolve([ ]);
    }
    if (!profilesQueryable) {
      return getProfiles(session, '/api/v1/profile.json', { count: sourceMaxCount });
    }
    var bookmark = last_known && last_known.profiles ? new Date(last_known.profiles) : null;
    var reads;
    if (bookmark && !isNaN(bookmark.getTime( ))) {
      reads = [
        getProfiles(session, '/api/v1/profiles.json', { find: { created_at: { $gt: bookmark.toISOString( ) }, startDate: { $gte: EPOCH } }, count: sourceMaxCount }),
        getProfiles(session, '/api/v1/profile.json', { count: 1 })
      ];
    } else {
      var windowStart = sinceFor(null, 'profiles').toISOString( );
      reads = [
        getProfiles(session, '/api/v1/profiles.json', { find: { startDate: { $gte: windowStart } }, count: sourceMaxCount }),
        getProfiles(session, '/api/v1/profiles.json', { find: { startDate: { $lt: windowStart } }, count: 1 })
      ];
    }
    return Promise.all(reads).then((lists) => {
      var seen = new Set( );
      return [ ].concat(...lists).filter((p) => {
        if (!p || p._id == null) return true;
      ...
      // Nightscout before 14 has no /api/v1/profiles; read the way earlier
      // connector versions did.
      if (requestError.status(err) !== 404) throw err;
      profilesQueryable = false;
      log.warn('The source Nightscout cannot filter profiles, so every profile is read on every poll. ' +
        'Updating the source Nightscout avoids this.');
      return getProfiles(session, '/api/v1/profile.json', { count: sourceMaxCount });
    });
  }
  // A reader subject created by an earlier nightscout-connect has `role`
  // instead of `roles`, so Nightscout gives it no roles and its reads are
  // refused. The connector reuses the subject by name and never changes the
  // source site, so it says once how to fix it. The message carries no
  // token, secret or URL.
  var READER_FIX = 'To fix this on the source Nightscout, open Admin Tools and either add the "readable" role ' +
    'to nightscout-connect-reader, or delete nightscout-connect-reader so nightscout-connect creates it again. ' +
    'nightscout-connect does not change the source site itself.';
  var READER_WARNINGS = {
    roleless: 'The source Nightscout already has a subject named nightscout-connect-reader, and it has no roles, ' +
      'so it cannot read any data. Earlier versions of nightscout-connect created it that way. ' + READER_FIX,
    unauthorized: 'The source Nightscout refused to let the subject named nightscout-connect-reader read data ' +
      '(HTTP 401). ' + READER_FIX
  };
  var readerWarned = { };
  var reusedReader = false;
  function warnReader (condition) {
    if (readerWarned[condition]) return;
    // A 401 from a role-less subject is the same problem, already reported.
    if (condition === 'unauthorized' && readerWarned.roleless) return;
    readerWarned[condition] = true;
    log.warn(READER_WARNINGS[condition]);
  }
  function getOrCreateReaderToken ( ) {
    ...
    return http.get(authURL, { headers }).then((resp) => {
      ...
      if (match) {
        reusedReader = true;
        if (!Array.isArray(match.roles) || !match.roles.length) warnReader('roleless');
  ...
  var impl = {
    ...
    dataFromSesssion(session, last_known) {
      return Promise.all([
        ...
        fetchProfiles(session, last_known)
      ]).then(([ entries, treatments, devicestatus, profiles ]) => ({ entries, treatments, devicestatus, profiles }),
        (err) => {
          if (reusedReader && requestError.status(err) === 401) warnReader('unauthorized');
          throw err;
        });
    },
