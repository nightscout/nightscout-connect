...
function carelinkSource (opts, axios) {
  ...
  var impl = {
    ...
    sessionFromAuth(account, settings) {
      ...
      function summarize ( ) {
        return Promise.all([getUser( ).then(getM2M).then(fetchPatientList), getProfile( ), getCountrySettings( ) ]).then((results) => {
    ...
    refreshSession (authInfo, session) {
      ...
      http.post({ url: modDefaults.refresh_token_url, headers }).then((resp) => {
        ...
        console.log("ERROR REFREESHING TOKEN!!");
        if (error.response) {
    ...
    align_to_glucose (last_known) {
      ...
      var buffer_lag = 58000; // 36 second buffer
  ...
  function generate_driver (builder) {
    builder.support_session({
      ...
      delays: {
        REFRESH_AFTER_SESSSION_DELAY: (1000 * 60 * 25),
        EXPIRE_SESSION_DELAY: 1000 * 60 *  35,
      }
    });
