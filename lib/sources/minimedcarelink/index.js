  ...
  var impl = {
    ...
    dataFromSesssion(session, last_known) {
      ...
      function summarize (results) {
        ...
        return results.filter((payload) => payload.sgs).pop( );
      }

      return Promise.all([ bleEndpointData( ), m2m_data( ) ]).then(summarize);
    },
    ...
    transformGlucose (data, last_known) {
      ...
      var entries = data.sgs.map(sgs_to_sgv).filter(is_missing);
