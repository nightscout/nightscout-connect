...
function v3PointToEntry (timestampDelta, timeZone, meterUnits, point) {
  ...
  // A graph x coordinate is already a Unix instant. Only the separate
  // timestamp field may be a Glooko local wall time encoded as UTC.
  var date = point.timestamp
    ? timezone.timestampWithOffset(timestamp, timestampDelta, timeZone)
    : new Date(point.x * 1000);
  return {
    type: 'sgv',
