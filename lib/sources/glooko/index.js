  ...
  var impl = {
    ...
    dataFromSesssion (session, last_known) {
      ...
      function fetcher (endpoint, params) {
        ...
        console.log('GLOOKO FETCHER LOADING');
      ...
      function fetchV2 (endpoint, params, collection) {
        return fetcher(endpoint, params).catch((err) => {
          ...
          cgmV2Error = err;
          console.warn('GLOOKO v2 request rejected', { status: Number.isInteger(status) ? status : 'unavailable' });
          return { [collection]: [ ] };
        });
      }
      ...
      */      
      ...
      function eventsFetcher (endpoint, optional) {
        ...
        return fetcher(endpoint, {
          ...
          console.log('GLOOKO wide fetch failed', { status: Number.isInteger(status) ? status : 'unavailable' });
         ...
         //console.log('settings sample', JSON.stringify(results[4]));
          ...
          if (!patientCode) {
            return fetcher('/api/v3/session/users', {})
              ...
              .catch((err) => {
                var status = err && err.response && err.response.status;
                console.log('GLOOKO V3 PROFILE/GRAPH FETCH FAILED', { status: Number.isInteger(status) ? status : 'unavailable' });
          ...
          return fetcher(constructV3GraphUrl(patientCode, new Date(two_days_ago), new Date( )), {})
            ...
            .catch((err) => {
              var status = err && err.response && err.response.status;
              console.log('GLOOKO V3 GRAPH FETCH FAILED', { status: Number.isInteger(status) ? status : 'unavailable' });
              if (cgmV2Error) return Promise.reject(err);
              return some;
            });
        });
    ...
    transformData (batch) {
      if (batch && batch.syncVersion === 1) {
        ...
        if (Object.keys(mapped.glookoSync.warnings).length || unavailable.length) {
          console.warn('GLOOKO SYNC WARNINGS', { count: Object.keys(mapped.glookoSync.warnings).length, unavailable: unavailable.length });
        }
        return mapped;
      }
      // TODO
