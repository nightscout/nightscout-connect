    ...
    dataFromSesssion (session, last_known) {
      ...
      function fetchV2 (endpoint, params, collection) {
        return fetcher(endpoint, params).catch((err) => {
          ...
          cgmV2Error = err;
          console.warn('GLOOKO v2 request rejected', endpoint.split('?')[0], 'status', status);
          return { [collection]: [ ] };
        });
      }
    ...
    transformData (batch) {
      if (batch && batch.syncVersion === 1) {
        ...
        if (Object.keys(mapped.glookoSync.warnings).length || unavailable.length) {
          console.warn('GLOOKO SYNC WARNINGS', { warnings: mapped.glookoSync.warnings, unavailable });
        }
        return mapped;
      }
      // TODO
      debug('GLOOKO passing batch for transforming');
