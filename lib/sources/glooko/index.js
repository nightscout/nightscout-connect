...
function authModeFor (value) {
  return [ 'api', 'v3', 'web', 'auto' ].includes(value) ? value : 'api';
}
...
function extractSessionCookie (headers) {
  var cookies = headers && (headers['set-cookie'] || headers['Set-Cookie']);
  var candidate = (Array.isArray(cookies) ? cookies : [cookies])
    .find((cookie) => typeof cookie === 'string' && cookie.startsWith('_logbook-web_session='));
...
function v3PointToEntry (timestampDelta, timeZone, meterUnits, point) {
  ...
  // Live cross-checks against raw EGV displayTime show that graph x carries
  // the same fake-UTC local clock, not an absolute Unix clinical instant.
  var date = timezone.timestampWithOffset(timestamp, timestampDelta, timeZone);
  return {
    type: 'sgv',
...
function getGlookoCode (session) {
  ...
  return user.userLogin && user.userLogin.glookoCode
    || user.user && user.user.glookoCode
...
function glookoSource (opts, axios, log) {
  ...
  var http = axios.create({ baseURL, headers: default_headers, timeout: 30000 });
  // Rejected Axios requests carry credentials, cookies and medical payloads.
  // Keep only transport/status information before errors reach the state machine.
  for (const method of ['get', 'post']) {
    if (typeof http[method] !== 'function') continue;
    const request = http[method].bind(http);
    http[method] = (...args) => Promise.resolve().then(() => request(...args)).catch(error => {
      if (!error || (!error.isAxiosError && !error.config)) throw error;
      const safe = new Error('Glooko request failed');
      safe.code = error.code;
      if (error.response) safe.response = { status: error.response.status };
      throw safe;
    });
  }
  var impl = {
    authFromCredentials ( ) {
      ...
      // Nocturne's Glooko connector uses the JSON v3 sign-in for accounts
      // whose v2 API sign-in is rejected; the patient code is fetched below.
      function v3Login ( ) {
        return http.post(Defaults.v3Login, {
          user: { email: opts.glookoEmail, password: opts.glookoPassword }
        }).then((response) => {
          assertNoTwoFactorRequired(response.data);
          var cookie = extractSessionCookie(response.headers);
          if (!cookie) {
            throw new Error('Glooko v3 sign-in did not return a session cookie.');
          }
      ...
      if (opts.glookoAuthMode === 'auto') {
        return apiLogin( ).catch((err) => {
          ...
          if (status === 422) {
            return v3Login( ).catch((v3Error) => {
              if (v3Error && v3Error.response && v3Error.response.status === 422) return webLogin( );
              throw v3Error;
            });
          }
    ...
    sessionFromAuth (auth) {
      if (getGlookoCode(auth)) return Promise.resolve(auth);
      var headers = { ...default_headers, Cookie: auth.cookies, Host: baseHost };
      return http.get('/api/v3/session/users', { headers, params: {} }).then((response) => {
        var session = { ...auth, userProfile: response.data };
        if (!getGlookoCode(session)) {
          throw new Error('Glooko session profile did not include a patient code.');
        }
        return session;
      });
    },
    dataFromSesssion (session, last_known) {
      if (!getGlookoCode(session)) {
        return Promise.reject(new Error('Glooko patient code is unavailable; cannot fetch data.'));
      }
      if (opts.glookoDataMode === 'sync') {
        return sync.collect((path, params) => http.get(path, {
          headers: { Cookie: session.cookies }, params, timeout: 30000
        }).then(response => response.data), session, opts, last_known);
      }
      ...
      function fetchV2 (endpoint, params, collection) {
        return fetcher(endpoint, params).catch((err) => {
          var status = err && err.response && err.response.status;
          if (collection !== 'readings' || status !== 422 || !opts.glookoUseV3Graph) {
            return Promise.reject(err);
          }
          cgmV2Error = err;
          log.warn('Glooko v2 ' + collection + ' request rejected' +
            (Number.isInteger(status) ? ' (HTTP ' + status + ')' : '') + '; using v3 graph');
          return { [collection]: [ ] };
        });
      }
      ...
      */      
      function constructUrl(endpoint) {
        ...
        log.debug('Glooko patient code ' + (patientCode ? 'resolved' : 'missing') +
          '; session user fields: ' + (session && session.user ? Object.keys(session.user).join(', ') : 'none'));
        if (!patientCode) {
          return null;
        }
      ...
      // `params` on purpose.
      function eventsFetcher (endpoint, optional) {
        ...
        return fetcher(endpoint, {
          ...
          if (!optional) return Promise.reject(e);
          log.debug('Glooko optional history request failed' +
            (e && e.response && Number.isInteger(e.response.status) ? ' (HTTP ' + e.response.status + ')' : ''));
      ...
      const v2Fetches = pumpBasalsUrl && pumpBolusUrl && cgmReadingsUrl
        ? [
            ...
            fetchV2(pumpBasalsUrl, treatmentParams, 'scheduledBasals'),
            fetchV2(pumpBolusUrl, treatmentParams, 'normalBoluses'),
            fetchV2(cgmReadingsUrl, entryParams, 'readings'),
            eventsFetcher(Defaults.PumpEvents),
            eventsFetcher(Defaults.PumpAlarms),
            eventsFetcher(Defaults.LatestPumpBasals, true),
      ...
      return Promise.all(v2Fetches).then(function (results) {
         ...
         var some = {
            ...
            normalBoluses: ((results[1] && results[1].normalBoluses) || [ ]).filter(function (b) {
              return b && !b.softDeleted && (!b.guid || !seen_guids.has(b.guid));
            }),
            ...
            wideBasals: ((results[5] && results[5].scheduledBasals && results[5].scheduledBasals.length
              ? results[5].scheduledBasals : results[0].scheduledBasals) || [ ]).filter(function (b) {
              if (!b || b.softDeleted || !b.pumpTimestamp || !b.guid) { return false; }
              return !seen_guids.has(b.guid);
            }),
         };
         // Counts only: alarm codes and pump events are the person's device history.
         if ((some.pumpAlarms && some.pumpAlarms.length) || (some.pumpEvents && some.pumpEvents.length)) {
           log.debug('Glooko new records: ' + (some.pumpAlarms || []).length + ' alarms, ' +
             (some.pumpEvents || []).length + ' pump events');
         }
         ...
         //console.log('settings sample', JSON.stringify(results[4]));
          ...
          // A non-empty v2 response may contain meter-style records (for
          // example bg_value/display_time) rather than convertible CGM data.
          // Fall back when none of its records can become Nightscout entries.
          if (!opts.glookoUseV3Graph || generate_nightscout_entries(
            some.readings, opts.glookoTimezoneOffset, opts.glookoTimezone
          ).length) {
            return some;
          }
          ...
          if (!patientCode) {
            return fetcher('/api/v3/session/users', {})
              ...
              .catch((err) => {
                ...
                if (cgmV2Error) return Promise.reject(err);
                return some;
              });
          }
    ...
    transformData (batch) {
      if (batch && batch.syncVersion === 1) {
        const mapped = mapper.transform(batch, opts);
        const unavailable = Object.entries(batch.diagnostics || {}).filter(([,value])=>value.unavailable).map(([key])=>key);
        if (Object.keys(mapped.glookoSync.warnings).length || unavailable.length) {
          const counts = Object.entries(mapped.glookoSync.warnings).map(([code, count]) => code + ' x' + count);
          log.warn('Glooko sync warnings: ' + (counts.join(', ') || 'none') +
      ...
      if (opts.glookoSkipEntries) {
        var only = { entries: [ ], treatments };
        var ds = helper.generate_nightscout_devicestatus(batch, opts.glookoTimezoneOffset, opts.glookoTimezone);
      ...
      var devicestatus = helper.generate_nightscout_devicestatus(batch, opts.glookoTimezoneOffset, opts.glookoTimezone);
...
glookoSource.validate = function validate_inputs (input) {
  ...
  var config = {
    ...
    glookoDataMode: input.glookoDataMode || 'sync',
    glookoLookbackDays: Number(input.glookoLookbackDays || 14),
    glookoImportProfile: boolish(input.glookoImportProfile),
    glookoExtendedBolusDurationUnit: input.glookoExtendedBolusDurationUnit,
    baseURL
  };
  var errors = [ ];
  if (!['sync', 'legacy'].includes(config.glookoDataMode)) {
    errors.push({ desc: 'CONNECT_GLOOKO_DATA_MODE must be sync or legacy.', err: new Error('CONNECT_GLOOKO_DATA_MODE') });
  }
  if (!Number.isInteger(config.glookoLookbackDays) || config.glookoLookbackDays < 1 || config.glookoLookbackDays > 90) {
    errors.push({ desc: 'CONNECT_GLOOKO_LOOKBACK_DAYS must be an integer from 1 to 90.', err: new Error('CONNECT_GLOOKO_LOOKBACK_DAYS') });
  }
  if (config.glookoExtendedBolusDurationUnit && !['seconds', 'minutes'].includes(config.glookoExtendedBolusDurationUnit)) {
    errors.push({ desc: 'CONNECT_GLOOKO_EXTENDED_BOLUS_DURATION_UNIT must be seconds or minutes.', err: new Error('CONNECT_GLOOKO_EXTENDED_BOLUS_DURATION_UNIT') });
  }
  ...
  if (!input.glookoServer && !_known_servers[input.glookoEnv || 'default']) {
    errors.push({
      desc: 'CONNECT_GLOOKO_ENV must name a supported region or CONNECT_GLOOKO_SERVER must be set.',
      err: new Error('CONNECT_GLOOKO_ENV')
    });
  }
  ok = errors.length == 0;
