  ...
  */
  ...
  if (pumpAlarms) {
    pumpAlarms.forEach(function (a) {
      ...
      treatment.eventTime = timezone.timestampWithOffset(a.pump_timestamp, timestampDelta, timeZone).toISOString( );
...
function generate_nightscout_devicestatus (batch, timestampDelta, timeZone) {
  ...
  pumpBoluses.forEach(function (element) {
    ...
    if (element.insulinOnBoard === undefined || element.insulinOnBoard === null) return;
    var iob = Number(element.insulinOnBoard);
    if (!isFinite(iob) || iob < 0) return;            // absent, or not a number
    if (!element.pumpTimestamp) return;
    var when = timezone.timestampWithOffset(element.pumpTimestamp, timestampDelta, timeZone);
