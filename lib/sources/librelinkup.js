...
function linkUpSource (opts, axios) {
  ...
  function resolveAuth(auth, steps) {
    ...
    if (redirect) {
      ...
      console.warn(`LibreLinkUp selected region ${nextRegion}; set CONNECT_LINK_UP_REGION=${nextRegion} to start there.`);
...
linkUpSource.validate = function validate_inputs (input) {
  ...
  var baseURL;
  var errors = [ ];
  var timezone = input.connectTimezone || input.timezone;
  var region = input.linkUpRegion;
  try {
    var inferred = transportOptions.regionForTimezone(timezone);
    if (!region && !input.linkUpServer) region = inferred || 'EU';
  } catch (_) {
    errors.push({ desc: 'CONNECT_TIMEZONE must be a valid IANA timezone', err: new Error('CONNECT_TIMEZONE') });
  }
