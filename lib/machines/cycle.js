...
function createCycleMachine (services, config) {
    ...
    delays: {
      ...
      EXPECTED_DATA_INTERVAL_DELAY: (context, event) => {
        ...
        if (context.align_to) {
          ...
          // Safety check: if the align_to time has already passed, use the default interval instead
          if (diff <= 0) {
            console.log("ALIGN_TO has passed, using default interval instead. diff:", diff);
  ...
  const loopMachine = Machine({
    ...
    states: {
      ...
      Operating: {
        entry: [
          logAction('Operating Invoking'),
        ],
        on: {
          // SESSION_RESOLVED and SESSION_ERROR should forward to the frame
          // only when the frame service is active (in Operating state)
          SESSION_RESOLVED: {
            actions: [
