...
//
// Both are configurable here and both default to 0, so a deployment that has
// not asked for jitter behaves as it does today: one actor is not a herd, and
// a self-hosted site should not wait out a problem it does not have. The
// hosted vendor pool is the deployment that sets them.
  ...
  const loopConfig = {
    ...
    delays: {
      START_JITTER_DELAY: (context, event) => {
        return jitter_ms(config.start_jitter_ms);
      },
      ...
      EXPECTED_DATA_INTERVAL_DELAY: (context, event) => {
        ...
        if (context.align_to) {
          ...
          // The source owns this one: align_to already carries whatever
          // spread that vendor's driver decided on, and adding more here
          // would push the fetch past the window the driver aimed at.
          return diff;
        }
        return config.expected_data_interval_ms + jitter_ms(config.interval_jitter_ms);
      }
    }
  };
