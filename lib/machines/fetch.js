...
function retryAfterMs(error) {
  var headers = error && error.response && error.response.headers;
  var value = headers && (headers['retry-after'] || headers['Retry-After']);
  if (!value) return 0;
  var seconds = Number(value);
  var duration = Number.isFinite(seconds) ? seconds * 1000 : Date.parse(value) - Date.now();
  return Number.isFinite(duration) ? Math.max(0, Math.min(duration, 15 * 60 * 1000)) : 0;
}
    ...
    guards: {
      shouldRetry: (context, event, transition) => {
        return context.retries < config.maxRetries &&
          !(config.noRetryStatuses || []).includes(context.last_error_status)
      }
    },
    delays: {
      ...
      */
      ...
      Fetching: {
        ...
        invoke: {
          ...
          onError: {
            ...
            actions: [
              actions.assign({
                last_error_status: (context, event) => event.data && event.data.response && event.data.response.status,
                last_error_retry_after_ms: (context, event) => retryAfterMs(event.data)
              }),
      ...
      Error: {
        entry: [
          ...
          actions.log('FRAME FAILED'),
          actions.sendParent((context) => ({
            type: 'FRAME_ERROR',
            status: context.last_error_status,
            retry_after_ms: context.last_error_retry_after_ms
          })),
        ],
        always: [
          {
