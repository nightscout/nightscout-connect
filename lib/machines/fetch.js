        ...
        on: {
          ...
          SESSION_ERROR: {
            ...
            actions: actions.assign({
              last_error_status: (context, event) => event.status,
              last_error_retry_after_ms: (context, event) => event.retry_after_ms || 0
            }),
          },
          SESSION_RESOLVED: {
      ...
      Fetching: {
        ...
        invoke: {
          ...
          onError: {
            ...
            actions: [
              actions.assign({
                last_error_status: (context, event) => requestError.status(event.data),
                last_error_retry_after_ms: (context, event) => requestError.retryAfterMs(event.data)
              }),
