...
function filesystemOutput (config, axios) {
  console.log("SETTING UP filesystemOutput");
  var target = {
  ...
  function record_batch (batch) {
    console.log("RECORD BATCH", { entries: (batch.entries || []).length, treatments: (batch.treatments || []).length, profiles: (batch.profiles || []).length, devicestatus: (batch.devicestatus || []).length });
    ...
    return fs.open(pathname, 'w+').then((fh) => {
      ...
      console.log("UPDATE BOOKMARK FROM I/O");
      return bookmark;
    });
