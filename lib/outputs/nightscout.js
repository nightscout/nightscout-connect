...
function nightscoutRestAPI (config, axios) {
  ...
  async function record_profiles (data) {
    ...
    var headers = { 'API-SECRET': apiHash };
    // Everything handled here, stored or already stored, moves the profile
    // bookmark the Nightscout source reads changes after.
    var handled = data;
    ...
    return http.post('/api/v1/profile.json', data, { headers }).then((resp) => {
      ...
      return handled.length > data.length ? handled : resp.data;
    }, (err) => {
