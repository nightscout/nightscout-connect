...
function nightscoutRestAPI (config, axios) {
  ...
  // stored must not be posted again. The Nightscout source re-reads
  // profiles, with their _id; Glooko profiles carry an identifier. A stored
  // profile whose content changed on the source is replaced with a PUT, when
  // that replaces it in place (see replaceable).
  const sync = require('./profile-sync');
  const profileKeys = sync.profileKeys;
  var notReplacedWarned = false;
  // PUT /api/v1/profile upserts the ObjectId form of the _id. Where the
  // destination holds the _id as a string (a Nightscout without the profile
  // _id fix stores a posted profile that way), that adds a second profile
  // beside the old one. find[_id] matches exactly what the PUT will replace:
  // the ObjectId form, and on a Nightscout with the fix the string form too.
  // So the profile is replaced only when that find returns it.
  async function replaceable (id, headers) {
    var found = await http.get('/api/v1/profiles.json', { headers, params: { find: { _id: String(id) }, count: 10 },
      paramsSerializer: p => require('qs').stringify(p) }).catch(recordingError);
    return Array.isArray(found.data) && found.data.length > 0;
  }
  async function replace_profile (p, headers) {
    var print = sync.fingerprint(p);
    if (await replaceable(p._id, headers)) {
      await http.put('/api/v1/profile.json', JSON.parse(JSON.stringify(p)), { headers }).catch(recordingError);
    } else if (!notReplacedWarned) {
      notReplacedWarned = true;
      log.warn(sync.NOT_REPLACED);
    }
    // Handled either way: the same version is not looked at again.
    sync.remember(knownProfiles, p, print);
  }
  async function record_profiles (data) {
    ...
    var headers = { 'API-SECRET': apiHash };
    // Everything handled here, stored, replaced or already stored, moves the
    // profile bookmark the Nightscout source reads changes after.
    var handled = data;
    var changed = [];
    try {
      if (data.some(p => profileKeys(p).length)) {
        if (!knownProfiles) {
          ...
          knownProfiles = sync.known(previous.data || []);
        }
        ({ create: data, replace: changed } = sync.plan(data, knownProfiles));
      }
      var stored = [];
      if (data.length) {
        var prints = data.map(sync.fingerprint);
        var resp = await http.post('/api/v1/profile.json', data, { headers }).catch(recordingError);
        if (knownProfiles) data.forEach((p, i) => sync.remember(knownProfiles, p, prints[i]));
        log.debug("Recorded " + resp.data.length + " profiles");
        stored = resp.data;
      }
      for (const p of changed) await replace_profile(p, headers);
      return handled.length > data.length ? handled : stored;
    } catch (err) {
  ...
  // The profile bookmark only moves forward: a poll also hands back the
  // newest profile by startDate, which can be older by created_at.
  function bookmark_newest (collection, field, data) {
    var newest = newestDate(data, field);
    if (newest && !(bookmark[collection] && bookmark[collection].getTime( ) >= newest.getTime( ))) {
      bookmark[collection] = newest;
    }
    return Promise.resolve(data);
  }
  ...
  function record_batch (batch) {
    ...
    */
    return Promise.all([
        ...
        record_profiles(profiles).then(bookmark_newest.bind(null, 'profiles', 'created_at'))
