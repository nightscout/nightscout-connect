...
function persistent (config, ctx) {
  ...
  ctx.bus.on('data-processed', function (sbx) {
    ...
    known = last;
    if (process.env.LOG_LEVEL === 'debug') console.log("DEBUG nightscout-connect", 'data-loaded',
  ...
  function persists (batch) {
    log.debug("INTERNAL PERSISTENCE", batch);
    ...
    */
    ...
    return Promise.all([
      ...
      log.debug("ALL SETTLED INTERNAL PERSIST", settled);
      return known;

    }).catch(( ) => {
      log.warn("PERSISTED INTERNAL ERRORED");
      return known;
    });
  ...
  persists.gap_for = function ( ) {
    ...
    function wait (resolve, reject) {
      ...
      function then ( ) {
        log.debug("EVENTUALLY FOUND", known);
        return resolve(known);
      }

    }
    log.debug("WAITING FOR data-processed");
