...
function persistent(config, ctx) {
  ...
  // stored must not be inserted again. The Nightscout source re-reads
  // profiles, with their _id; Glooko profiles carry an identifier. A stored
  // profile whose content changed on the source is replaced through the
  // profile save, when that replaces it in place (see replaceable).
  const sync = require('./profile-sync');
  const profileKeys = sync.profileKeys;
  let notReplacedWarned = false;
  ...
  async function loadProfiles() {
    ...
    knownProfiles = sync.known(stored);
    advanceProfiles(stored);
  }
  // The profile save upserts the ObjectId form of the _id. Where the stored
  // copy holds the _id as a string (a Nightscout without the profile _id fix
  // stores a copied profile that way), that adds a second profile beside the
  // old one. A find by _id matches exactly what the save will replace: the
  // ObjectId form, and on a Nightscout with the fix the string form too. So
  // the profile is replaced only when that find returns it.
  function replaceable(id) {
    if (typeof ctx.profile.list_query !== 'function' || typeof ctx.profile.save !== 'function')
      return Promise.resolve(false);
    return new Promise((resolve, reject) =>
      ctx.profile.list_query({ find: { _id: String(id) }, count: 10 },
        (err, docs) => (err ? reject(err) : resolve(Array.isArray(docs) && docs.length > 0))));
  }
  async function replace(p) {
    const print = sync.fingerprint(p);
    if (await replaceable(p._id)) {
      await new Promise((resolve, reject) =>
        ctx.profile.save(JSON.parse(JSON.stringify(p)), (err) => (err ? reject(err) : resolve())));
  ...
  async function profiles(rows) {
    ...
    if (rows.some((p) => profileKeys(p).length)) {
      ...
      ({ create: rows, replace: changed } = sync.plan(rows, knownProfiles));
    }
    const prints = rows.map(sync.fingerprint);
    let saved;
    try {
      saved = await create('profile', rows);
      if (knownProfiles) rows.forEach((p, i) => sync.remember(knownProfiles, p, prints[i]));
      for (const p of changed) await replace(p);
    } catch (err) {
