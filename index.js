...
function manage (env, ctx) {
  ...
  console.log("INPUT PARAMS", { kind: validated.config.kind, baseURL: validated.config.baseURL });

  if (!validated.ok) {
    console.log("Invalid, disabling nightscout-connect", validated.errors.map(function (error) { return error.desc; }));
    return;
  }
